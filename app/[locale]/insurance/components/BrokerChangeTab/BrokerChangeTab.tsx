...
import { IBrokerChangeForm, TFormStatus } from "@/types/form";
...
function BrokerChangeTab () {
    ...
    const statuses = {
      idle: <>
        ...
        <Button
          ...
          {status === 'pending' ? <Spinner data-icon="inline-start"/> : <RefreshCwIcon className='w-5 h-5'/>}
          {t('submit')}
        </Button></>,
  ...
  }, [status, brokerChangeSubmitButtonDisabled, brokerChangeForm, brokerChangeFormValid])
