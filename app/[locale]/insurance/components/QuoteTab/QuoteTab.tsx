...
import { IQuoteForm, TFormStatus } from "@/types/form";
...
function QuoteTab () {
    ...
    const statuses = {
      idle: <>
        ...
        <Button type="submit" disabled={!quoteSubmitButtonDisabled || status === 'pending'}
          ...
          {status === 'pending' ? <Spinner data-icon="inline-start"/> : <SendIcon className='w-5 h-5'/>}
          {t('submit')}
        </Button></>,
  ...
  }, [status, quoteSubmitButtonDisabled, quoteForm, quoteFormValid])
