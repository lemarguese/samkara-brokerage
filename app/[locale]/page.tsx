...
const advantages = (t: ReturnType<typeof createTranslator<any, any>>) => [
  { icon: <DollarSignIcon className="w-5 h-5 text-black" />, title: t('Trust.list.0.title'), description: t('Trust.list.0.description') },
  { icon: <ZapIcon className="w-5 h-5 text-black" />,        title: t('Trust.list.1.title'), description: t('Trust.list.1.description') },
  { icon: <AwardIcon className="w-5 h-5 text-black" />,      title: t('Trust.list.2.title'), description: t('Trust.list.2.description') },
  { icon: <HeartIcon className="w-5 h-5 text-black" />,      title: t('Trust.list.3.title'), description: t('Trust.list.3.description') },
  { icon: <LanguagesIcon className="w-5 h-5 text-black" />,  title: t('Trust.list.4.title'), description: t('Trust.list.4.description') },
];
  ...
  return (
    <div className="font-[Outfit,sans-serif]">
      ...
      <section className="pt-20 md:pt-24 bg-zinc-950 relative overflow-hidden min-h-[600px]">
        ...
        <div className="hero-glow-b absolute -bottom-20 -right-20 w-[600px] h-[600px] rounded-full pointer-events-none" />
          ...
          <div className="ticker-track flex whitespace-nowrap w-max">
            {[...Array(2)].map((_, pass) =>
              ['Rideshare Coverage','Same-Day TLC','Fleet Insurance',
                ...
                <span key={`${pass}-${i}`} className="ticker-item inline-flex items-center gap-4 px-9 font-bold text-xs tracking-widest uppercase text-black">
                  {item}
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              <div className="flex flex-wrap gap-2 mb-6">
                {[
                  ...
                  <span key={p.label} className={`hero-pill hero-pill-${p.i} ${p.cls} px-3 py-1 rounded-full text-xs font-bold tracking-wide uppercase`}>
                    {p.label}
                  </span>
                ))}
              </div>

              <h1 className="hero-h1-anim text-5xl md:text-6xl lg:text-7xl font-black text-white leading-[0.95] mb-6 tracking-tight">
                {t.rich("Top.title", {
              ...
              <ul className="hero-checks-anim space-y-3 mb-8">
                {[t('Top.advantages.first'), t('Top.advantages.second'), t('Top.advantages.third')].map((adv, i) => (
                  <li key={i} className="flex items-center gap-3 text-white">
                    <span className="w-5 h-5 rounded-full bg-yellow-400/15 border border-yellow-400 flex items-center justify-center flex-shrink-0">
            ...
            <div className="hero-card-anim hidden lg:block">
              <div className="relative">
                <div className="bg-gradient-to-br from-zinc-800 to-zinc-900 rounded-3xl p-7 border border-yellow-400/12 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    />
                    <div className="absolute -top-3 -right-3 bg-yellow-400 text-black px-4 py-2 rounded-xl font-bold text-xs shadow-lg flex items-center gap-1.5 tracking-wide uppercase">
                      <CarIcon className="w-3.5 h-3.5" /> {t('Top.tlc_approved')}
                    </div>
                    <div className="absolute -bottom-3 -left-3 bg-pink-500 text-white px-4 py-2 rounded-xl font-bold text-xs shadow-lg flex items-center gap-1.5 tracking-wide uppercase">
                  ...
                  <div className="mt-8 grid grid-cols-3 gap-3 text-center">
                    {[
                      ...
                      <div key={s.lbl} className="bg-zinc-800/80 rounded-xl p-4 border border-white/5 hover:border-yellow-400/25 hover:bg-yellow-400/4 transition-all cursor-default">
      ...
      <section id="insurance" className="py-20 md:py-28 bg-zinc-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-yellow-400/15 text-yellow-700 px-4 py-2 rounded-full text-xs font-bold mb-4 uppercase tracking-wider">
            ...
            <p className="text-base text-zinc-500 max-w-2xl mx-auto font-light leading-relaxed">{t('Service_Area.description')}</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services(t).map((service, idx) => (
              <div key={idx} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 border border-zinc-100 group cursor-pointer">
                <div className={`bg-gradient-to-br ${service.banner} h-20 flex items-center px-6 relative overflow-hidden`}>
                  <div className={`${service.icon.background} ${service.icon.text} w-12 h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                    {service.icon.value}
                  </div>
                  <div className={`absolute right-6 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full ${service.dot} opacity-60 animate-[pulse_2s_ease-in-out_infinite]`} />
                </div>
                <div className="p-6">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <div className="flex items-center gap-2 mb-5">
                {[
                  ...
                  <span key={p.label} className={`${p.cls} px-3 py-1 rounded-full text-xs font-bold tracking-wide uppercase`}>{p.label}</span>
                ))}
              </div>
              ...
              <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 rounded-2xl p-6 border border-yellow-400/12 relative overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-yellow-400/40 to-transparent" />
                <div className="text-6xl font-black text-yellow-400/10 leading-none mb-[-16px]">"</div>
                <div className="flex items-start gap-4 relative z-10">
                  <img alt="Insurance Agent" className="w-14 h-14 rounded-full object-cover border-2 border-yellow-400/40 flex-shrink-0"
                       src="https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=100&q=80" />
                  <div>
                    <p className="text-zinc-400 italic mb-3 text-sm leading-relaxed font-light">{t('Trust.quote.text')}</p>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {advantages(t).map((adv, i) => (
                <div key={i} className={`flex gap-4 p-5 rounded-2xl border border-zinc-100 hover:border-yellow-400/30 hover:bg-yellow-50/50 hover:-translate-y-0.5 transition-all duration-200 cursor-pointer ${i === 4 ? 'sm:col-span-2' : ''}`}>
                  <div className="w-11 h-11 bg-gradient-to-br from-yellow-400 to-yellow-500 rounded-xl flex items-center justify-center flex-shrink-0 shadow-[0_4px_16px_rgba(246,201,14,0.25)]">
                    {adv.icon}
                  </div>
                  <div>
      ...
      <section id="testimonials" className="py-20 md:py-28 bg-zinc-950 relative overflow-hidden">
        <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-transparent via-yellow-400 to-transparent opacity-60" />
        <div className="absolute top-[3px] left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-pink-500/50 to-transparent" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-5 mb-12">
            {testimonials.map((testi, i) => (
              <div key={i} className={`bg-white/[0.03] border border-white/7 rounded-2xl p-7 relative overflow-hidden group ${testi.cardHoverBorder} hover:-translate-y-1 transition-all duration-300 cursor-pointer`}>
                <div className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r ${testi.topLine} opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase ${testi.platformStyle} mb-4`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${testi.dotColor} animate-[pulse_2s_ease-in-out_infinite]`} />
                  {testi.platform}
                </div>
                ...
                <p className="text-zinc-400 text-sm leading-[1.8] mb-6 font-light">
                  {testi.quote.split(testi.highlight).map((part, idx, arr) => (
                    idx < arr.length - 1
                      ? <span key={idx}>{part}<strong className="text-white font-semibold">{testi.highlight}</strong></span>
                      : <span key={idx}>{part}</span>
                  ))}
                </p>
                <div className="flex items-center gap-3 pt-4 border-t border-white/6">
                  <img alt={testi.name} className="w-10 h-10 rounded-full object-cover border-2 border-yellow-400/30" src={testi.avatar} />
                  <div>
                  ...
                  <div className="ml-auto bg-yellow-400/8 border border-yellow-400/15 rounded-full px-2.5 py-1 text-[10px] text-yellow-400/60 font-medium">
                    ✓ Verified
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white/[0.02] border border-white/7 rounded-2xl px-8 py-7 flex flex-wrap justify-center items-center gap-0">
            {[
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {contacts(t).map((contact, i) => (
              <div key={i} className="bg-white rounded-2xl border border-zinc-200 p-6 text-center hover:-translate-y-1 hover:shadow-lg hover:border-yellow-400/30 transition-all duration-200 cursor-pointer">
                <div className={`w-12 h-12 ${contact.icon.background} rounded-xl flex items-center justify-center mx-auto mb-4`}>
                  {contact.icon.value}
                </div>
                <h3 className="font-bold text-zinc-900 mb-2 text-sm uppercase tracking-wide">{contact.title}</h3>
                {contact.href
                  ? <a href={contact.href} className={`${contact.content.style} hover:underline`}>{contact.content.value}</a>
                  : <p className={contact.content.style}>{contact.content.value}</p>
                }
              </div>
            ))}
          </div>
          <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 rounded-2xl p-10 text-center relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-yellow-400 to-transparent" />
            <div className="absolute bottom-0 right-0 w-64 h-64 rounded-full bg-yellow-400/4 blur-3xl pointer-events-none" />
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href="/insurance" className="inline-flex items-center gap-2 bg-yellow-400 text-black px-7 py-3.5 rounded-xl font-bold hover:bg-yellow-300 transition-colors shadow-[0_4px_20px_rgba(246,201,14,0.3)]">
                Get Started
              </a>
              <a href="tel:+12123145555" className="inline-flex items-center gap-2 bg-white/8 text-white px-7 py-3.5 rounded-xl font-semibold hover:bg-white/15 transition-colors border border-white/12">
